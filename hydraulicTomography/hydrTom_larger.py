...
def myUTruth(x, y):
		...
		return -1*np.logical_and(y <= 10+3/16*x, True) +  (-4)*np.logical_and(y <= 60-0.25*x, y > 10 + 3/16*x) + (2)*np.logical_and(y>60-0.25*x, np.logical_and(y<=0.8*x, y > 10+3/16*x)) + 2.5 * np.logical_and(y <= -20 + 0.8*x, np.logical_and(y <= 30-0.25*x, y<=5 + 3/16*x)) + (-3.5)*np.logical_and((x-120)**2+4*(y-50)**2 <= 400, y>=45)
def u_D_term(x, y):
	return x*0
...
uOpt = u0
start = time.time()
D = invProb.DI_adjoint_vec_wavelet(uOpt)
end = time.time()
print("time for one gradient: " + str(end-start))
for kk in range(3):
