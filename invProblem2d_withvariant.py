...
class inverseProblem():
	...
	def Phi(self, u, obs=None, obspos=None, Fu=None, variant=False):
		if obs is None:
			assert(self.obs is not None)
			obs = self.obs
		discrepancy = obs-self.Gfnc(u, Fu, obspos=obspos)
		if variant:
			return 1/(2*self.gamma**2*len(obs))*np.dot(discrepancy,discrepancy) 
		return 1/(2*self.gamma**2)*np.dot(discrepancy,discrepancy) 
		
		
	def DPhi(self, u, h, obs=None, obspos=None, Fu=None, variant=False):
		if obs is None:
			assert(self.obs is not None)
			obs = self.obs
		...
		if variant:
			return -1.0/(self.gamma**2*len(obs))*np.dot(discrepancy, DG_of_u_h)		
	...
	def D2Phi(self, u, h1, h2=None, obs=None, obspos=None, Fu=None):		
		...
		if variant:
			return 1.0/(self.gamma**2*len(obs)) * np.dot(DG_of_u_h1, DG_of_u_h2) - 1.0/(self.gamma**2*len(obs))*np.dot(discrepancy, D2G_of_u_h1h2)
		return 1.0/self.gamma**2 * np.dot(DG_of_u_h1, DG_of_u_h2) - 1.0/self.gamma**2*np.dot(discrepancy, D2G_of_u_h1h2)
	
	def I(self, u, obs=None, obspos=None, Fu=None, variant=False):
		return self.Phi(u, obs, obspos=obspos, Fu=Fu, variant=variant) + self.prior.normpart(u)
	
	def DI(self, u, h, obs=None, obspos=None, Fu=None, variant=False):
		DPhi_u_h = self.DPhi(u, h, obs=obs, obspos=obspos, Fu=Fu, variant=variant)
		inner = self.prior.covInnerProd(u, h)	
		return DPhi_u_h + inner
	
	def DI_vec_wavelet(self, u, obs, obspos=None, Fu=None, variant=False):
		...
		for direction in range(numDir):
			...
			DIvec[direction] = self.DI(u, h, obs, obspos=obspos, Fu=Fu, variant=variant)
		return DIvec
		
	def DI_vec_fourier(self, u, obs, obspos=None, Fu=None, variant=False):
	...
	def D2I(self, u, h1, h2=None, obs=None, variant=False):
		D2Phi_u_h1_h2 = self.D2Phi(u, h1, h2=h2, obs=obs, variant=variant)
		if h2 is None:
			return D2Phi_u_h1_h2 + self.prior.covInnerProd(h1, h1)
		else:
			return D2Phi_u_h1_h2 + self.prior.covInnerProd(h1, h2)
	
	def D2I_mat_wavelet(self, u, obs, obspos=None, Fu=None, variant=False):		
		numDir = unpackWavelet(u.waveletcoeffs).shape[0]
		...
		for dir1 in range(numDir):
			...
			for dir2 in range(dir1, numDir):
				...
				D2Imat[dir1, dir2] = self.D2I(u, h1, h2=h2, obs=obs, variant=variant)
				D2Imat[dir2, dir1] = D2Imat[dir1, dir2]
		return D2Imat
	
	def D2I_mat_fourier(self, u, obs, obspos=None, Fu=None, variant=False):		
		numDir = len(u.fouriercoeffs.flatten())
		D2Imat = np.zeros((numDir,numDir))
		N = self.prior.N
		for dir1 in range(numDir):
	...
	def I_forOpt(self, u_modes_unpacked, variant=False):
	...
	def DI_forOpt(self, u_modes_unpacked, variant=False):
		if isinstance(self.prior, GaussianFourier2d):
			return self.DI_vec_fourier(mor.mapOnRectangle(self.rect, "fourier", u_modes_unpacked.reshape((self.prior.N, self.prior.N))), self.obs, variant=variant)
		elif isinstance(self.prior, GeneralizedGaussianWavelet2d):
			return self.DI_vec_wavelet(mor.mapOnRectangle(self.rect, "wavelet", packWavelet(u_modes_unpacked)), self.obs, variant=variant)
		else:
			raise Exception("not a valid option")
	
	def find_uMAP(self, u0, nit=5000, nfev=5000, method='Nelder-Mead', variant=False):
		assert(self.obs is not None)
		start = time.time()
		u0_vec = None
		if isinstance(self.prior, GaussianFourier2d):
		...
		if method=='Nelder-Mead':
			If = lambda u: self.I_forOpt(u, variant=variant)
			...
			rate = 0.0001
			If = lambda u: self.I_forOpt(u*rate, variant=variant)
			DIf = lambda u: rate*self.DI_forOpt(u*rate, variant=variant)
		...
		print("Reduction of function value from " + str(self.I(u0, variant=variant)) + " to " + str(self.I(uOpt, variant=variant)))
		return uOpt
