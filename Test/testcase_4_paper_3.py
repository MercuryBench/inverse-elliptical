...
m1 = GeneralizedGaussianWavelet2d(rect_coarse, 0.1, 1.0, rect_coarse.resol+1)
m1 = GeneralizedWavelet2d(rect_coarse, 0.1, 1.0, rect_coarse.resol+1, p=1.001)
m2 = GeneralizedGaussianWavelet2d(rect_coarse, 0.1, 2.0, rect_coarse.resol+1)
...
if len(sys.argv) > 1:
	...
	uOpt = u0
	uu = m2.sample()
	uu = mor.mapOnRectangle(rect, "wavelet", uu.waveletcoeffs)
	#start=time.time();uList, uListUnique, PhiList = invProb.randomwalk_pCN(uu, 200, beta=0.0001);end=time.time();print(end-start); plt.figure(); plt.plot(PhiList)
	
	#uOpt = invProb.find_uMAP(uOpt, nit=50, method = 'BFGS', adjoint=True, version=0);invProb.plotSolAndLogPermeability(uOpt)
	
	
	m2 = GaussianFourier2d(rect, np.zeros((33,33)), 1.0, 6.0)
	invProb2 = inverseProblem(fwd, m2, gamma)
	invProb2.obs = obs
	invProb2.obspos=obspos
	uOpt2 = mor.mapOnRectangle(rect, "fourier", m2._mean)
	uOpt3 = invProb2.find_uMAP(uOpt2, nit=200, method = 'BFGS', adjoint=False);
	filename = "uMAP_fourier_manyobs_2.png";
	invProb2.plotSolAndLogPermeability(uOpt2, save=filename)
