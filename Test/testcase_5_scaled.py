...
resol = 6
rect = Rectangle((0,0), (100,10), resol=resol)
gamma = 0.01

def myUTruth_unscaled(x, y):
...
def myUTruth(x,y):
	return myUTruth_unscaled(0.01*x,0.1*y)
def u_D_term(x, y):
	return x*0
...
f = mor.mapOnRectangle(rect, "handle", lambda x, y: ((0.01*x-0.75)**2 + (0.1*y-0.5)**2 < 0.05**2)*(-20.0))
...
if len(sys.argv) > 1:
	...
	obspos = [100*obspos[0,:], 10*obspos[1, :]]
	invProb.obspos = obspos
