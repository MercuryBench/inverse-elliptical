...
rect = Rectangle((0,0), (1,1), resol=8)
rectCoarse = Rectangle((0,0), (1,1), resol=7)
gamma = 0.005
...
fCoarse = mor.mapOnRectangle(rectCoarse, "handle", lambda x, y: (((x-.8)**2 + (y-.2)**2) < 0.2**2)*(20.0))
f = mor.mapOnRectangle(rect, "wavelet", parseResolution(fCoarse.waveletcoeffs, rectCoarse.resol))

fwd = linEllipt2dRectangle(rect, f, u_D, boundary_D_boolean)
m1 = GeneralizedGaussianWavelet2d(rect, 0.01, 1.5, 7) # 0.01, 1.5
...
#uTruth = m1.sample()
uTruthCoarse = mor.mapOnRectangle(rectCoarse, "handle", lambda x, y: myUTruth2(x, y)) # was: rect instead of rectCoarse!
uTruth = mor.mapOnRectangle(rect, "wavelet", parseResolution(uTruthCoarse.waveletcoeffs, rectCoarse.resol)) # was: rectCoarse.resol instead of rect.resol!
