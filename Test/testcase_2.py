...
def u_D_term(x, y):
	return np.logical_and(x >= 0.5, y <= 0.625)*(x-0.6)*(0.5-y)*200
...
m1 = GeneralizedGaussianWavelet2d(rect, 0.1, 2.0, 6)
...
if len(sys.argv) > 1:
	...
	uOpt = invProb.find_uMAP(u0, nit=150, method = 'BFGS', adjoint=True, version=0)
	invProb.plotSolAndLogPermeability(uOpt)
