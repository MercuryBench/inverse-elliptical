...
uOpt3 = mor.mapOnRectangle(rect, "wavelet", m1._mean)

for l in range(N_opt):
	DPhi = invProb.DPhi_adjoint_vec_wavelet(mor.mapOnRectangle(invProb.rect, "wavelet", uOpt3.waveletcoeffs), version=0)
	# uTemp = uOpt2 - mor.mapOnRectangle(rect, "wavelet", packWaveletDPhi)*tau
	alpha = linesearch(uOpt3, mor.mapOnRectangle(invProb.rect, "wavelet", packWavelet(DPhi)))
	temp = shrinkage(unpackWavelet(uOpt3.waveletcoeffs) - alpha*DPhi, alpha)
	uOpt3 = mor.mapOnRectangle(rect, "wavelet", packWavelet(temp))
	valI3[l] = invProb.I(uOpt3)
	valPhi3[l] = invProb.Phi(uOpt3)
...
for l in range(N_opt):
	...
	valI2[l] = invProb.I(uOpt2)
	valPhi2[l] = invProb.Phi(uOpt2)
...
tks = np.zeros((N_opt,))
tks[1] = 1
valI[0] = invProb.I(mor.mapOnRectangle(rect, "wavelet", packWavelet(xk)))
valPhi[0] = invProb.Phi(mor.mapOnRectangle(rect, "wavelet", packWavelet(xk)))
...
for l in range(1, N_opt):
	DPhi = invProb.DPhi_adjoint_vec_wavelet(mor.mapOnRectangle(invProb.rect, "wavelet", packWavelet(yk)), version=0)
	# uTemp = uOpt2 - mor.mapOnRectangle(rect, "wavelet", packWaveletDPhi)*tau
	alpha = linesearch(mor.mapOnRectangle(invProb.rect, "wavelet", packWavelet(yk)), mor.mapOnRectangle(invProb.rect, "wavelet", packWavelet(DPhi)))
	xk = shrinkage(yk - alpha*DPhi, alpha)
	if (l <= N_opt-2):
		tks[l+1] = 0.5 * (1.0 + sqrt(1.0 + 4.0*tks[l]**2))
		ykp1 = xk + (tks[l]-1)/tks[l+1] * (xk - xkm1)
	uOpt = mor.mapOnRectangle(rect, "wavelet", packWavelet(xk))
	valI[l] = invProb.I(uOpt)
	valPhi[l] = invProb.Phi(uOpt)
...
plt.figure()
plt.subplot(211)
plt.semilogy(valI2, 'g--', label="I_ISTA")
plt.semilogy(valI3, 'r--', label="I_ISTA_backtr")
plt.semilogy(valI4, 'm--', label="I_FISTA");
plt.semilogy(valI, 'k--', label="I_FISTA_backtr");
plt.legend()
plt.subplot(212)
plt.semilogy(valPhi, 'k', label="Phi_FISTA_backtr")
plt.semilogy(valPhi3, 'r', label="Phi_ISTA_backtr")
...
print("squared Cameron-Martin norm of linesearch-ISTA optimizer: " + str(invProb.prior.normpart(uOpt3)))
print("squared misfit of linesearch-ISTA optimizer: " + str(invProb.Phi(uOpt3, obs)))
...
print("squared Cameron-Martin norm of linesearch-FISTA optimizer: " + str(invProb.prior.normpart(uOpt)))
print("squared misfit of linesearch-FISTA optimizer: " + str(invProb.Phi(uOpt, obs)))
