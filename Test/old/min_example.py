...
f = Expression("pow(x[0]-0.4, 2) + pow(x[1]-0.3, 2) <= 0.05 ? -40.0 : 0.0", degree=2)
...
boundary_conditions = {4: {'Dirichlet': Constant(0.0)}, 1: {'Neumann':   Constant(0)}, 2: {'Dirichlet': Constant(1)}, 3: {'Dirichlet':   Expression("x[0]", degree=2)}}
...
boundary_conditions2 = {4: {'Dirichlet': Constant(0.0)}, 1: {'Neumann':   Constant(0.0)}, 2: {'Dirichlet': Constant(0.0)}, 3: {'Dirichlet':  Constant(0.0)}}
...
print(uSol(p1,p2))
term1 = dot(grad(uSol2), n)*Expression("x[0]", degree=2)*ds2(4) + dot(grad(uSol2), n)*Expression("x[0]", degree=2)*ds2(2) + dot(grad(uSol2), n)*Expression("x[0]", degree=2)*ds2(3)
