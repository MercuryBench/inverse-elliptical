...
#rect_fine = Rectangle((0,0), (1,1), resol=resol+1)
...
#fwd_fine = linEllipt2dRectangle(rect_fine, f_fine, u_D_fine, boundary_D_boolean)
m1 = GeneralizedGaussianWavelet2d(rect_coarse, 0.0001, 1.0, rect_coarse.resol+1)
invProb = inverseProblem(fwd, m1, gamma)
#invProb_fine = inverseProblem(fwd_fine, m1, gamma)
...
if len(sys.argv) > 1:
	...
	for kk in range(25):
		uOpt = invProb.find_uMAP(uOpt, nit=120, method = 'BFGS', adjoint=True, version=0);
		invProb.plotSolAndLogPermeability(uOpt)
