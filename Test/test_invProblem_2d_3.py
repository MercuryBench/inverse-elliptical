...
m1 = GeneralizedGaussianWavelet2d(rect, 0.1, 1.5, 5)
...
start=time.time()
uOpt_adj = u0;
uOpt_adj = invProb.find_uMAP(uOpt_adj, nit=400, method = 'BFGS', adjoint=True, version=0)
end=time.time()
...
uList, uListUnique, PhiList = invProb.randomwalk_pCN(uStart, 100, beta=0.0001, showDetails=True)
uListMALA, uListUniqueMALA, PhiListMALA = invProb.randomwalk_MALA(uStart, 100, beta=0.0001, showDetails=True)
plt.figure(); plt.plot(PhiList, label="pCN"); plt.plot(PhiListMALA, label="MALA");plt.legend()
