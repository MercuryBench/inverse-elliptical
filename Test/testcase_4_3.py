...
def myUTruth(x, y):
		...
		return 2*np.logical_and(y <= 0.3-0.1*x, True) + (-1)*np.logical_and(y>0.3-0.1*x, y<=0.8*x) + (-3)*np.logical_and(y > 0.3-0.1*x, np.logical_and(y > 0.8*x, y<=0.7-0.15*x))
def u_D_term(x, y):
	return x*0
...
def myUTruth(x, y):
		...
		return np.logical_or(np.logical_and(np.logical_and((x+0.5)**2 + (y-0.2)**2 <= 1.2, (x+0.5)**2 + (y-0.25)**2 >= 1), y <=0.3), np.logical_and(np.logical_and((x+0.5)**2 + (y-0.2)**2 <= 1.2, (x+0.5)**2 + (y-0.25)**2 >= 1), y >=0.5))*(-20.0)
...
f = mor.mapOnRectangle(rect, "handle", lambda x, y: ((x-0.2)**2 + (y-0.75)**2 < 0.05**2)*(-400.0)) + mor.mapOnRectangle(rect, "handle", lambda x, y: ((x-0.65)**2 + (y-0.2)**2 < 0.05**2)*(400.0))

fwd = linEllipt2dRectangle(rect, f, u_D, boundary_D_boolean)
m1 = GeneralizedGaussianWavelet2d(rect, 0.05, 1.8, resol)
...
if len(sys.argv) > 1:
	...
	N_obs = 9
	obspos = [np.array([0.2, 0.25, 0.3, 0.35, 0.4, 0.3, 0.4, 0.45, 0.5]), np.array([0.95, 0.9, 0.85, 0.75, 0.65, 0.95, 0.85, 0.75, 0.65])]
	invProb.obspos = obspos
	...
	uOpt = invProb.find_uMAP(u0, nit=50, method = 'BFGS', adjoint=True, version=0); invProb.plotSolAndLogPermeability(uOpt)
