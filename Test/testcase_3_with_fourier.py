...
invProb1 = inverseProblem(fwd, m1, gamma)
...
if len(sys.argv) > 1:
	...
	obs = invProb1.Gfnc(uTruth1) + np.random.normal(0, gamma, (N_obs,))
	invProb1.plotSolAndLogPermeability(uTruth1, obs=obs)
	invProb2.plotSolAndLogPermeability(uTruth2)
	invProb1.obs = obs
	invProb2.obs = obs
	#u0 = mor.mapOnRectangle(rect, "wavelet", m1._mean)
	u01 = mor.mapOnRectangle(rect, "wavelet", m1._mean)+3
	m2mean = m2._mean 
	m2mean[0,0] = 3
...
"""end = time.time()
print("Took " + str(end-start) + " seconds, which is " + str((end-start)/(3600.0)) + " hours.")
data = {"obs": obs, "obspos": obspos, "resol": resol, "gamma": gamma, "u0_waveletcoeffs": u0.waveletcoeffs, "uOpt_waveletcoeffs": uOpt.waveletcoeffs, "uOptFourier_fouriermodes": uOpt2.fouriermodes}
